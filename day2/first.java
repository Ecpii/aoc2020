package day2;

import java.io.File;
import java.util.Scanner;
import java.io.IOException;

public class first {
    public static void main(String[] args) throws IOException {
        Scanner in = new Scanner(new File("./day2/input.txt"));
        int validPasswords = 0;

        while (in.hasNext()) {
            String passwordLine = in.nextLine();
            int charOccurances = 0;
            boolean validPassword = true;

            int hyphenIndex = passwordLine.indexOf('-');
            int minOccurances = Integer.parseInt(passwordLine.substring(0, hyphenIndex));
            passwordLine = passwordLine.substring(hyphenIndex + 1);

            int spaceIndex = passwordLine.indexOf(' ');
            int maxOccurances = Integer.parseInt(passwordLine.substring(0,  spaceIndex));
            passwordLine = passwordLine.substring(spaceIndex + 1);

            char desiredChar = passwordLine.charAt(0);
            String password = passwordLine.substring(passwordLine.indexOf(' ') + 1);

            for (char testChar : password.toCharArray()) {
                if (testChar == desiredChar) {
                    charOccurances++;
                }
            }
            validPassword = charOccurances >= minOccurances && charOccurances <= maxOccurances;
            if (validPassword) {
                validPasswords++;
            }
        }
        System.out.println(validPasswords);
    }
}
